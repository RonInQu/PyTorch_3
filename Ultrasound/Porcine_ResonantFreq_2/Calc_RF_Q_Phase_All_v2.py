...
input_file = os.path.join(data_dir, '/BL1.TXT')   # <-- change this
# ============================================================
...
output_plot    = os.path.join(data_dir, f'{base_name}_crystal_GB_Phase_BvD.png')
...
# Phase angle (degrees) from G and B
# φ = atan2(B, G)  →  range (−180°, +180°)
phase_deg = np.degrees(np.arctan2(B, G))
...
# ------------------------------------------------------------------
# B-curve features (still computed for the results file)
# ------------------------------------------------------------------
i_Bmax = np.argmax(B)
f_Bmax = freq[i_Bmax]
Bmax = B[i_Bmax]
...
# Zero-phase crossing (closest point to 0° near resonance)
# Useful for PLL / zero-phase tracking discussion
idx_near = np.argmin(np.abs(freq - fs))
# search in a window around resonance
win = max(5, int(0.02 * len(freq)))
lo, hi = max(0, idx_near - win), min(len(freq), idx_near + win)
i_zero = lo + np.argmin(np.abs(phase_deg[lo:hi]))
f_zero = freq[i_zero]
phase_zero = phase_deg[i_zero]
...
results.append(f'Phase at fs     = {phase_at_fs:.2f} °')
results.append('')
...
results.append('')
results.append('========== Phase features ==========')
results.append(f'Phase at fs     = {phase_at_fs:.2f} °')
results.append(f'Nearest zero-phase freq = {f_zero:.3f} Hz  ({f_zero/1e6:.6f} MHz)   φ = {phase_zero:.2f} °')
results.append('')
...
# ------------------------------------------------------------------
# Plot – top: G, bottom: Phase angle
# ------------------------------------------------------------------
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)

# Top panel – Conductance
...
ax1.plot(fs/1e6, Gmax*1000, 'ro', markersize=7)
...
# Bottom panel – Phase angle
ax2.plot(freq/1e6, phase_deg, 'm-', lw=1.2, label='Phase φ = atan2(B, G)')
ax2.axhline(0, color='k', lw=0.8, alpha=0.7)
ax2.axvline(fs/1e6, color='g', ls='--', alpha=0.7, label='fs (G peak)')
ax2.plot(fs/1e6, phase_at_fs, 'k^', markersize=8, label=f'φ at fs = {phase_at_fs:.1f}°')
ax2.plot(f_zero/1e6, phase_zero, 'go', markersize=7, label=f'Nearest 0° @ {f_zero/1e6:.4f} MHz')
ax2.set_xlabel('Frequency (MHz)')
ax2.set_ylabel('Phase angle φ (degrees)')
ax2.legend(loc='best')
ax2.grid(True)

# Optional: tighten y-limits around the resonance region for clarity
# (comment out if you prefer the full ±180° range)
ph_win = phase_deg[lo:hi]
if len(ph_win) > 0:
    y_pad = 15
    ax2.set_ylim(np.min(ph_win) - y_pad, np.max(ph_win) + y_pad)
