"""
Batch 2 inverted-only: split 25 inverted studies (clot_max > wall_max) into train/test.
...
for fname in sorted(os.listdir('v94_data_set/training')):
    ...
    # Inverted = clot_mean > wall_mean AND clot_max > wall_max
...
print(f"Inverted studies with clot_max > wall_max: {len(df_r)}")

# Pick ~6 for test (best tissue coverage), rest for train
test_studies = df_r.head(6)['study'].tolist()
