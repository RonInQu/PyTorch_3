...
class  ClotFeatureExtractor:
    ...
    def _compute(self, data, ema_fast, ema_slow):
        ...
        # NOTE: Computed on DENOISED signal (1.5s median filter removes cardiac pulse)
        #       so features reflect tissue morphology, not heartbeat oscillations.
        if self._need_shape:
            # Remove cardiac pulse: 1.5s median filter extracts slow trend
            _SHAPE_MED_KERNEL = int(self.fs * 1.5) | 1  # 225 samples, odd
            if n >= _SHAPE_MED_KERNEL:
                data_smooth = medfilt(data, kernel_size=_SHAPE_MED_KERNEL)
            else:
                data_smooth = data

            d_diff = np.diff(data_smooth)
            ...
            if data_std > 0.01:
                peaks, _ = find_peaks(data_smooth, prominence=data_std * 0.3)
            ...
            if n > kernel * 2:
                trend = np.convolve(data_smooth, np.ones(kernel) / kernel, 'same')
                detr = data_smooth[kernel // 2: -(kernel // 2)] - trend[kernel // 2: -(kernel // 2)]
