...
# Discrete benchmark data points from benchtop measurements
# Freq in kHz -> Impedance |Z| in ohms
MEASURED_BENCHMARKS = {
    "TWW_B": {
        "label": "Blood Points",
        "color": "#2ca02c",  # Green
        "marker": "o",
        "freq_khz": [12.5, 50.0, 100.0],
        "impedance_ohm": [1319, 1377, 1359],
    },
    "TWW_C": {
        "label": "Clot Points",
        "color": "#d62728",  # Red
...
def parse_4294a_txt(file_path: Path) -> pd.DataFrame:
    ...
    for raw in lines:
        ...
        if line.startswith('"TRACE:'):
            ...
            if trace_label in ("A", "B"):
                current_trace = trace_label
            else:
                current_trace = None
            in_table = False
            continue
    ...
    if not trace_data["A"] or not trace_data["B"]:
        raise ValueError(f"Missing Trace A/B data in {file_path}")
...
def plot_overlay_points(ax: plt.Axes, stem: str) -> None:
    """Helper function to plot discrete benchmark points on an impedance axis."""
    for key, data in MEASURED_BENCHMARKS.items():
        if key in stem:
            ax.scatter(
                data["freq_khz"],
                data["impedance_ohm"],
            ...
            # Annotate each value above the marker
            for f, z in zip(data["freq_khz"], data["impedance_ohm"]):
                ax.annotate(
                    f"{z:.0f} Ω",
                    (f, z),
...
def make_plots(df: pd.DataFrame, out_dir: Path, stem: str) -> None:
    ...
    ax1.plot(freq_khz, df["impedance_ohm"].values, color="#333333", linewidth=2, label="Sweep |Z|")
    plot_overlay_points(ax1, stem)
    ax1.set_xscale("log")
    ...
    ax2.plot(freq_khz, df["phase_deg"].values, color="#d62728", linewidth=2, label="Sweep Phase")
    ax2.set_xscale("log")
    ...
    # Separate Impedance plot with data points overlaid
    fig_i, ax_i = plt.subplots(figsize=(8.8, 4.6))
    ax_i.plot(freq_khz, df["impedance_ohm"].values, color="#333333", linewidth=2, label="Sweep |Z|")
    plot_overlay_points(ax_i, stem)
    ax_i.set_xscale("log")
