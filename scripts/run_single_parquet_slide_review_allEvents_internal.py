...
from scripts.build_slide_style_similarity_views_AllEvents_internal import plot_file_slide_style_from_df
...
def map_da_label_series(led_series: pd.Series) -> pd.Series:
...
def assign_numeric_label_series(event_series: pd.Series) -> pd.Series:
...
def resolve_truth_columns(df: pd.DataFrame, event_cols: list[str]) -> pd.DataFrame:
...
def build_testing_output(df_cropped: pd.DataFrame, time_col: str, resistance_col: str) -> pd.DataFrame:
    ...
    if "da_label_resolved" in df_testing.columns:
        df_testing["da_label"] = df_testing["da_label_resolved"]
    elif "curr_led_state" in df_testing.columns:
    ...
    if "resolved_label" in df_testing.columns:
...
def build_training_view(df_cropped: pd.DataFrame, time_col: str, resistance_col: str, min_tissue_duration_sec: float | None) -> pd.DataFrame:
    ...
    if "da_label_resolved" in df_training.columns:
        df_training["da_label"] = df_training["da_label_resolved"]
    elif "curr_led_state" in df_training.columns:
        df_training["da_label"] = df_training["curr_led_state"].apply(map_da_label)
    else:
        df_training["da_label"] = 0

    event_col = "resolved_event" if "resolved_event" in df_training.columns else "event_type_1"
    if "resolved_label" in df_training.columns:
...
def save_graphics(
    ...
    if "da_label_resolved" in df_graphics.columns:
        df_graphics["da_label"] = df_graphics["da_label_resolved"]
    elif "curr_led_state" in df_graphics.columns:
        df_graphics["da_label"] = df_graphics["curr_led_state"].apply(map_da_label)
    else:
        df_graphics["da_label"] = 0

    event_col = "resolved_event" if "resolved_event" in df_graphics.columns else "event_type_1"
...
def process_one_file(input_parquet: Path, output_root: Path, min_tissue_duration_sec: float | None) -> dict[str, Path]:
    ...
    df_cropped, _ = crop_to_blood_range(df1, "resolved_event", time_col)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run single raw parquet through internal all-events review output generation. Outputs only graphics and slide_style_similarity folders."
    )
