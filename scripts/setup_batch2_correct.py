"""
Batch 2 normal-only: clot_mean < wall_mean (same filter as Batch 1).
Same 8 test studies as Batch 1. All v94 normal-polarity studies go to training.
NO clot_max > wall_max filter — that was wrong.
"""
...
# Find all normal studies (clot_mean < wall_mean) — same filter as Batch 1
eligible_train = []
for fname in sorted(os.listdir('v94_data_set/training')):
    ...
    # Normal = clot_mean < wall_mean (ONLY filter, same as Batch 1)
...
print(f"This:    {len(train_studies)} train / 8 test (same test, same filter, more data)")
