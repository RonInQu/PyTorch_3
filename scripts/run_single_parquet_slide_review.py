...
from scripts.build_slide_style_similarity_views import plot_file_slide_style
...
def save_graphics(
    ...
    plt.figure(figsize=(14, 7))
    time_sec = df_training_out["timeInMS"].to_numpy(dtype=np.float64) / 1000.0
    r_vals = df_training_out["magRLoadAdjusted"].to_numpy(dtype=np.float32)
    labels = df_training_out["label"].to_numpy(dtype=np.int16)
...
def process_one_file(input_parquet: Path, output_root: Path, min_tissue_duration_sec: float | None) -> dict[str, Path]:
    ...
    testing_dir = output_root / "testing"
    graphics_dir = output_root / "graphics"
    slide_dir = output_root / "slide_style_similarity"

    for folder in [training_dir, testing_dir, graphics_dir, slide_dir]:
    ...
    df_training_out = build_training_output(df_cropped, time_col, resistance_col, min_tissue_duration_sec)
    df_testing_out = build_testing_output(df_cropped, time_col, resistance_col)

    training_path = training_dir / f"{study_name}_labeled_segment.parquet"
    testing_path = testing_dir / f"{study_name}_labeled_segment.parquet"
    df_training_out.to_parquet(training_path, index=True)
    df_testing_out.to_parquet(testing_path, index=True)

    save_graphics(
        study_name=study_name,
        df_cropped=df_cropped,
        df_training_out=df_training_out,
        time_col=time_col,
    ...
    plot_file_slide_style(study_name, testing_dir, slide_png)

    return {
        "training_parquet": training_path,
        "testing_parquet": testing_path,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run single raw parquet through LabelingWithDuration-style labeling and slide-style review plot generation."
    )
    ...
    parser.add_argument(
        ...
        help="Minimum clot/wall duration used by the training-view labeling branch.",
    )
