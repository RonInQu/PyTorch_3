"""
Batch 4 (April 10 data): clot_mean < wall_mean filter, same 8 test studies as Batch 1.
Source: vApril10_data_set/training → training_data/, vApril10_data_set/testing → test_data/
"""
...
all_studies = []
eligible_train = []
for fname in sorted(os.listdir('vApril10_data_set/training')):
    path = os.path.join('vApril10_data_set/training', fname)
...
print(f"Total studies in vApril10_data_set: {n_total}")
...
for s in test_studies:
    exists = os.path.exists(os.path.join('vApril10_data_set', 'testing', f'{s}_labeled_segment.parquet'))
...
for s in train_studies:
    src = os.path.join('vApril10_data_set', 'training', f'{s}_labeled_segment.parquet')
    dst = os.path.join('training_data', f'{s}_labeled_segment.parquet')
    shutil.copy2(src, dst)

copied_test = 0
for s in test_studies:
    src = os.path.join('vApril10_data_set', 'testing', f'{s}_labeled_segment.parquet')
    dst = os.path.join('test_data', f'{s}_labeled_segment.parquet')
    if os.path.exists(src):
        shutil.copy2(src, dst)
        copied_test += 1
    else:
        print(f"  WARNING: {s} not found in vApril10_data_set/testing/")

print(f"Copied {len(train_studies)} training files (vApril10/training → training_data/)")
print(f"Copied {copied_test} test files (vApril10/testing → test_data/)")
...
print(f"Batch 3: 34 train / 8 test")
