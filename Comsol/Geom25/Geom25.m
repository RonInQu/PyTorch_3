...
% 50 kHz baseline from current Geom25 geometry (user-verified run)
Z_meas50_blood = 790;
Z_meas50_clot  = 3440;
Z_meas50_wall  = 1774;

% Calibrated conductivities [S/m]
% Start from geometric K estimate, then apply a small per-tissue correction
% so 50 kHz lands on exact targets for this updated geometry.
sigma_blood = (K_real / Z_target_blood) * (Z_meas50_blood / Z_target_blood);
sigma_clot  = (K_real / Z_target_clot)  * (Z_meas50_clot  / Z_target_clot);
sigma_wall  = (K_real / Z_target_wall)  * (Z_meas50_wall  / Z_target_wall);
sigma_cath  = 1e-10;
...
geom.feature('imp1').set('filename', ...
    fullfile(pwd, 'SA-2001 - ASPIRATION TIP II, SA_V3.STEP'));
