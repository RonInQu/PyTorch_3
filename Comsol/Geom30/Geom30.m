...
fprintf('=== COMSOL LiveLink - Real Geometry Model (Gen 3.0) ===\n');
fprintf('  Date: %s\n', datestr(now));
...
vessel_length  = 0.040;
vessel_z_start = -0.015;

% Domain / Boundary IDs
% With 3 imported catheter objects + 2 cylinders → expect 5 domains
% Verify in COMSOL GUI after geometry builds. Typical numbering:
%   1=wall (outer annulus), 2=blood (inner minus cath), 3,4,5=catheter parts
dom_wall  = 1;
dom_blood = 2;
dom_cath  = [3, 4, 5];   % three catheter objects from STEP import
bnd_elec_L = 175;          % verify in GUI after import
bnd_elec_R = 37;         % verify in GUI after import

% Cell constant
% K_real = 702;   % m^-1
K_real = 728*0.8775; % m^-1  This makes impedance equal to that of geom25
...
% --- Domain numbering ---
% With 3 imported catheter objects + 2 cylinders, COMSOL creates multiple
% domains. We MUST assign materials to ALL domains or get:
%   "Undefined material property 'sigma' required by Current Conservation"
%
% Strategy: assign catheter material to ALL unassigned domains (safe:
% sigma_cath ~ 0 won't affect impedance results).
n_domains = geom.getNDomains();
n_boundaries = geom.getNBoundaries();
fprintf('  Geometry finalized: %d domains, %d boundaries.\n', n_domains, n_boundaries);

dom_all = 1:n_domains;
dom_assigned = unique([dom_wall, dom_blood, dom_cath]);
dom_unassigned = setdiff(dom_all, dom_assigned);

if ~isempty(dom_unassigned)
    fprintf('  WARNING: %d unassigned domain(s): [%s]\n', ...
        length(dom_unassigned), num2str(dom_unassigned));
    fprintf('  -> Assigning catheter material to unassigned domains.\n');
    dom_cath = unique([dom_cath, dom_unassigned]);
end

fprintf('  Domain assignment: Blood=[%s], Wall=[%s], Cath=[%s]\n', ...
    num2str(dom_blood), num2str(dom_wall), num2str(dom_cath));
...
% =====================================================================
try
    mphsave(model, fullfile(pwd, 'Geom30_complete.mph'));
    fprintf('  Model saved: Geom30_complete.mph\n');
catch
    alt_name = sprintf('Geom30_%s.mph', datestr(now, 'yyyymmdd_HHMMSS'));
