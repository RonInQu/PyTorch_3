# fit_scaler_V6_Normalized.py
"""
Fits the StandardScaler for clot detection features — V6 Normalized.
...
# ================= IMPORT FROM gru_torch_V6_Normalized =================
from src.models.gru_torch_V6_Normalized import FEATURE_SET, TOTAL_FEATURES, \
    ClotFeatureExtractor, SEQ_LEN, WINDOW_SEC, active_idx, active_dim, dim_str

from src.training.train_gru_V6_Normalized import STRIDE_SAMPLES

# ================= Derived CONFIG =================
OUTPUT_SCALER_PATH = PROJECT_ROOT / "src" / "data" / f"clot_feature_scaler_normalized_5s_seq{SEQ_LEN}_{dim_str}.pkl"

print("Scaler config (NORMALIZED):")
...
print(f"   Input column    = R_normalized (dimensionless)")
...
# lfilter coefficients for vectorized EMA
...
def _extract_one_file(file_path):
    ...
    required_cols = ['timeInMS', 'R_normalized', 'label']
    ...
    resistance = df['R_normalized'].to_numpy(dtype=np.float32)
    ...
    # Per-worker extractor instance
    ext = ClotFeatureExtractor(sample_rate=SAMPLE_RATE, window_sec=WINDOW_SEC,
                               active_features=active_idx)

    # Precompute full-run EMA arrays (cumulative)
...
plt.title(f"Feature Correlation Heatmap — Normalized ({X_scaled.shape[1]} features)")
plt.tight_layout()
plt.savefig("feature_correlation_heatmap_normalized.png", dpi=300, bbox_inches='tight')
plt.close()
print("   Saved → feature_correlation_heatmap_normalized.png")
