# -*- coding: utf-8 -*-
"""
LabelingWithDivisionNormalized.py — Duration-filtered labeling with division normalization.

Based on LabelingWithDuration.py but uses division normalization:
    R_norm = (R_raw - baseline) / baseline

This makes the signal dimensionless and patient/device-independent:
    blood ≈ 0 (near baseline)
    clot  ≈ 0.5–3.0 (moderate rise above baseline)
    wall  ≈ 1.0–5.0+ (large rise above baseline)

Advantages over subtraction+offset (R - baseline + 800):
    - Removes inter-patient baseline variability
    - Features become scale-invariant
    - Consistent interpretation across studies

Outlier threshold: R_norm > OUTLIER_THRESHOLD (default 6.0 = 6× baseline).
Blanking value: noise around 0 (since blood ≈ 0 after normalization).

Training: ALL non-tissue events are blanked to noise~0 (label=0).
Short tissue events (< MIN_TISSUE_DURATION_SEC) are also blanked.
Only sustained clot/wall events keep their real normalized values.

Testing: ALL data kept including artifacts. R_norm > OUTLIER_THRESHOLD clipped.
...
NOISE_VALUE = 0.01  # Small noise around 0 for blanked regions (dimensionless)
SAMPLE_RATE = 150

# Outlier threshold in normalized units (R_norm > this → outlier)
# 6.0 means signal is 6× above baseline (e.g., contrast spikes)
OUTLIER_THRESHOLD = 6.0

# Minimum tissue event duration (seconds). Clot/wall events shorter than this
# are blanked in training.
MIN_TISSUE_DURATION_SEC = 7.0

output_base = os.path.join(input_folder, 'processedResults_normalized')
...
print(f"Normalization: (R_raw - baseline) / baseline")
...
# Events that represent real tissue contact
...
def blank_short_tissue_events(df, resistance_col, label_col, min_dur_sec):
    """Blank clot/wall events shorter than min_dur_sec to noise~0.
    ...
    for cls in [1, 2]:  # clot, wall
        ...
        for s, e in zip(starts, ends):
            if (e - s) < min_samples:
                ...
                df.loc[idx, resistance_col] = noise
...
for file_path in parquet_files:
    ...
    # Division normalization: (R_raw - baseline) / baseline
    # Guard against zero/near-zero baseline with a floor
    baseline = df1[baseline_col_name].astype('float64')
    baseline_safe = baseline.clip(lower=1.0)  # avoid division by zero
    
    df1['R_normalized'] = (df1[imp_col_name].astype('float64') - baseline) / baseline_safe
    resistance_col = 'R_normalized'

    # Report normalization stats
    blood_mask_tmp = df1[event_col].isin(blood_events)
    if blood_mask_tmp.any():
        blood_norm = df1.loc[blood_mask_tmp, resistance_col]
        print(f"  Blood R_norm: mean={blood_norm.mean():.4f}, std={blood_norm.std():.4f}, "
              f"median={blood_norm.median():.4f}")
    
    clot_mask_tmp = df1[event_col].isin(clot_events)
    if clot_mask_tmp.any():
        clot_norm = df1.loc[clot_mask_tmp, resistance_col]
        print(f"  Clot  R_norm: mean={clot_norm.mean():.4f}, std={clot_norm.std():.4f}")
    
    wall_mask_tmp = df1[event_col].isin(wall_events)
    if wall_mask_tmp.any():
        wall_norm = df1.loc[wall_mask_tmp, resistance_col]
        print(f"  Wall  R_norm: mean={wall_norm.mean():.4f}, std={wall_norm.std():.4f}")
    ...
    def assign_numeric_label(event):
        ...
        return 0   # unrecognized events → blood
    ...
    outlier_mask  = df_training[resistance_col] > OUTLIER_THRESHOLD
    blank_mask    = (~tissue_mask) | outlier_mask

    n_non_tissue = (~tissue_mask).sum()
    n_outlier_train = outlier_mask.sum()
    print(f"  Non-tissue events blanked: {n_non_tissue} ({n_non_tissue/len(df_training)*100:.1f}%)")
    print(f"  Outliers (R_norm > {OUTLIER_THRESHOLD}): {n_outlier_train}")

    if blank_mask.any():
        noise = NOISE_VALUE * np.random.randn(blank_mask.sum())
        df_training.loc[blank_mask, resistance_col] = noise
    ...
    if MIN_TISSUE_DURATION_SEC:
        n_evt, n_samp = blank_short_tissue_events(
            df_training, resistance_col, 'label', MIN_TISSUE_DURATION_SEC)
        if n_evt > 0:
    ...
    df_training_out = pd.DataFrame({
        ...
        'R_normalized': df_training[resistance_col],
    ...
    print(f"  Saved training (normalized, artifacts + short events blanked)")
    ...
    if first_time is not None:
        ...
        outlier_mask = (df_graphics[resistance_col] > OUTLIER_THRESHOLD) & (~df_graphics[event_col].isin(artifact_events))
        blank_mask = default_blank_mask | outlier_mask

        if blank_mask.any():
            noise = NOISE_VALUE * np.random.randn(blank_mask.sum())
            df_graphics.loc[blank_mask, resistance_col] = noise
        ...
        plt.title(f"{study_name} — Division Normalized (R-baseline)/baseline\n"
                  f"blood≈0, clot≈0.5-3, wall≈1-5+ | Outlier threshold={OUTLIER_THRESHOLD}")
        plt.xlabel('Time (seconds)')
        plt.ylabel('R_normalized (dimensionless)')
        ...
        # ── Training-view graphic ──
        train_label_colors = {0: ('black', 'blood'), 1: ('red', 'clot'), 2: ('blue', 'wall')}

        plt.figure(figsize=(14, 7))
        time_ms = df_training_out['timeInMS'].values / 1000.0
        r_vals  = df_training_out['R_normalized'].values
        ...
        plt.title(f"{study_name} — Training View (normalized)\n"
                  f"After blanking: artifacts, outliers, short events (<{MIN_TISSUE_DURATION_SEC}s) → 0")
        plt.xlabel('Time (seconds)')
        plt.ylabel('R_normalized (dimensionless)')
        plt.grid(True, alpha=0.3)
        plt.legend(loc='upper right')
    ...
    # ===================================================================
    # 3. TESTING: Keep ALL data, clip R_norm > OUTLIER_THRESHOLD
    # ===================================================================
    ...
    # Clip outliers (not blanked, just capped)
    n_outlier = (df_testing[resistance_col] > OUTLIER_THRESHOLD).sum()
    df_testing[resistance_col] = df_testing[resistance_col].clip(upper=OUTLIER_THRESHOLD)
    ...
    print(f"  Test: {n_non_tissue} non-tissue events KEPT ({n_non_tissue/len(df_testing)*100:.1f}%), "
          f"{n_outlier} outliers clipped to {OUTLIER_THRESHOLD}")

    # Assign labels — no duration filter on test data
    ...
    df_testing_out = pd.DataFrame({
        ...
        'R_normalized': df_testing[resistance_col],
    ...
    print(f"  Saved testing (normalized, R_norm>{OUTLIER_THRESHOLD} clipped, full duration)")

print(f"\nAll files processed!")
print(f"  Normalization: (R_raw - baseline) / baseline")
print(f"  Training: non-tissue blanked to ~0, short events (<{MIN_TISSUE_DURATION_SEC}s) blanked.")
print(f"  Testing: all data kept, R_norm>{OUTLIER_THRESHOLD} clipped, NO duration filter.")
print(f"  Output column: 'R_normalized' (dimensionless)")
