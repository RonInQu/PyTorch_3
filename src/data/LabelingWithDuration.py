...
Short tissue events (< MIN_TISSUE_DURATION_SEC) blanked same as training.
...
for file_path in parquet_files:
    ...
    # Blank short-duration tissue events (< MIN_TISSUE_DURATION_SEC) — same as training
    if MIN_TISSUE_DURATION_SEC:
        blood_median_test = df_testing.loc[df_testing[event_col].isin(blood_events), resistance_col].median()
        if pd.isna(blood_median_test):
            blood_median_test = df_testing[resistance_col].median()
        n_evt, n_samp = blank_short_tissue_events(
            df_testing, resistance_col, 'label', blood_median_test, MIN_TISSUE_DURATION_SEC)
        if n_evt > 0:
            print(f"  Short events blanked (testing): {n_evt} events, {n_samp} samples "
                  f"(< {MIN_TISSUE_DURATION_SEC}s)")
    ...
    print(f"  Saved testing (R>5000 clipped, short events blanked)")

print(f"\nAll files processed!")
print(f"  Training: non-tissue blanked, short events (<{MIN_TISSUE_DURATION_SEC}s) blanked.")
print(f"  Testing: all data kept, R>5000 clipped, short events (<{MIN_TISSUE_DURATION_SEC}s) blanked.")
