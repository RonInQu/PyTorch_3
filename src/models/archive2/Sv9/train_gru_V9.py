...
# train_gru_V9.py
"""
Training script for clot detection — V9 (2-class: clot vs wall).
...
# Import from gru_torch_V9 (2-class ClotGRU with output_size=2)
from src.models.gru_torch_V9 import ClotFeatureExtractor, ClotGRU, \
...
def load_or_extract_features(force_extract: bool = False):
    ...
    cache_filename = f"features_V9_2class_w{WINDOW_SEC:.1f}s_s{STRIDE_SAMPLES}_seq{SEQ_LEN}_{FEATURE_SET}.npz"
    ...
    if not SCALER_PATH.exists():
        ...
        print("Run src/data/fit_scaler_V9.py first.")
        sys.exit(1)
...
def main():
    ...
    for seed in SEEDS_TO_TRY:
        ...
        if best_state_this_seed is not None:
            model_filename = f"clot_gru_trained_V9_2class_seq{SEQ_LEN}_{FEATURE_SET}_seed{seed}_f1{best_f1_this_seed:.4f}.pt"
            save_path = PROJECT_ROOT / "src" / "training" / model_filename

            torch.save(best_state_this_seed, save_path)

            print(f"   ✅ Saved V9 model for seed {seed}")
    ...
    if best_state_global is not None:
        latest_path = PROJECT_ROOT / "src" / "training" / f"clot_gru_trained_V9_2class_seq{SEQ_LEN}_{FEATURE_SET}.pt"
        torch.save(best_state_global, latest_path)
        print(f"\n✅ Also saved overall best as: clot_gru_trained_V9_2class_seq{SEQ_LEN}_{FEATURE_SET}.pt")

        generic_path = PROJECT_ROOT / "src" / "training" / "clot_gru_trained_V9_2class.pt"
        torch.save(best_state_global, generic_path)
        print(f"✅ Also saved as: clot_gru_trained_V9_2class.pt (default inference path)")

    print("\n" + "="*70)
    print("V9 TRAINING FINISHED")
    print("="*70)
