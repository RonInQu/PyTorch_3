# #%%
# train_gru_V10.py
"""
Training script for clot detection — V10 (2-class: clot vs wall).

V9 architectural change from V6:
  - Model output is 2 classes (clot, wall). Blood is dropped from training
    because DA blood is unconditional at inference (V6 rule).
  - Rationale: ~86% of V6 training was blood, which is dead weight for the
    2-class problem the model actually has to solve at inference. Freeing the
    model from blood should help it use its capacity for the harder clot/wall
    distinction at high resistance.
  - Label mapping: source parquet labels {1: clot, 2: wall} → target {0: clot, 1: wall}.
    Windows labeled blood (0) are skipped entirely.
  - Wall is the minority class after removing blood; balanced class weights
    naturally upweight wall (~1.9× clot).

Cache, scaler, and model artifacts all use distinct V9 filenames so V6/V9
can coexist.
"""

import os
import random
import sys
from pathlib import Path

import matplotlib
matplotlib.use('Agg')

import numpy as np
import pandas as pd
import torch
import torch.nn as nn
from torch.optim.lr_scheduler import ReduceLROnPlateau
from torch.utils.data import TensorDataset, DataLoader

import joblib
from joblib import Parallel, delayed
from scipy.signal import lfilter
from sklearn.model_selection import GroupKFold
from sklearn.utils.class_weight import compute_class_weight
from sklearn.metrics import f1_score, confusion_matrix

# Add project root to Python path
PROJECT_ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(PROJECT_ROOT))

# Import from gru_torch_V10 (2-class ClotGRU with output_size=2)
from src.models.gru_torch_V10 import ClotFeatureExtractor, ClotGRU, \
    FEATURE_SET, SEQ_LEN, WINDOW_SEC, \
    active_idx, active_dim, dim_str

# ────────────────────────────────────────────────
# CONFIGURATION
# ────────────────────────────────────────────────

SEEDS_TO_TRY = [456]

STRIDE_SAMPLES = 30

BATCH_SIZE = 1024
N_EPOCHS = 100
PATIENCE = 15
LR = 0.0001
WEIGHT_DECAY = 1e-4
SMOOTHING_FACTOR = 0.0

# Loss function: "cross_entropy" or "focal"
LOSS_FN = "cross_entropy"
FOCAL_GAMMA = 2.0

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

NUM_WORKERS = 8
PIN_MEMORY = True

os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'
os.environ["PYARROW_IGNORE_TIMEZONE"] = "1"

# Paths (all V9-specific to avoid clobbering V6)
DATA_DIR = PROJECT_ROOT / "training_data"
TEST_DIR = PROJECT_ROOT / "test_data"

SCALER_PATH = PROJECT_ROOT / "src" / "data" / f"clot_feature_scaler_V10_2class_seq{SEQ_LEN}_{dim_str}.pkl"
CACHE_DIR = PROJECT_ROOT / "cache"

# V9 is 2-class: [clot, wall]. No blood class.
CLASS_NAMES = ['clot', 'wall']
NUM_CLASSES = 2
# Clinical weight floor per class. Kept at 1.0 so that balanced class weights
# (which will already upweight wall as the minority class) drive the training.
CLINICAL_WEIGHTS = [1.0, 1.0]

# ────────────────────────────────────────────────
# Extractor + lfilter setup (precomputed once)
# ────────────────────────────────────────────────
_extractor = ClotFeatureExtractor(sample_rate=150, window_sec=WINDOW_SEC,
                                  active_features=active_idx)
WINDOW_SAMPLES = _extractor.window_size
ALPHA_FAST = _extractor.alpha_fast
ALPHA_SLOW = _extractor.alpha_slow

_B_FAST = np.array([ALPHA_FAST])
_A_FAST = np.array([1.0, -(1.0 - ALPHA_FAST)])
_B_SLOW = np.array([ALPHA_SLOW])
_A_SLOW = np.array([1.0, -(1.0 - ALPHA_SLOW)])

# ────────────────────────────────────────────────
# Utility Functions
# ────────────────────────────────────────────────

def set_print_options():
    pd.set_option('display.max_columns', None)
    pd.set_option('display.width', 1000)


def print_cm_text(y_true, y_pred, title, class_names=CLASS_NAMES, normalize=False):
    cm = confusion_matrix(y_true, y_pred, labels=list(range(len(class_names))))
    print(f"\n{title}")
    print(" " * 10 + " ".join(f"{name:>8}" for name in class_names))
    for i, row in enumerate(cm):
        print(f"{class_names[i]:<10}" + " ".join(f"{v:8}" for v in row))

    if normalize:
        cm_norm = cm.astype('float') / np.maximum(cm.sum(axis=1)[:, np.newaxis], 1)
        print(f"\nNormalized {title}:")
        print(" " * 10 + " ".join(f"{name:>8}" for name in class_names))
        for i, row in enumerate(cm_norm):
            print(f"{class_names[i]:<10}" + " ".join(f"{v:8.3f}" for v in row))


def print_label_stats_table(y_true, y_pred, title):
    """V9: only clot and wall."""
    class_names = ['Clt', 'Wall']
    print(f"\n{title}")
    print("-" * 82)
    print(f"{'Label':<6} {'TP':>6} {'FN':>6} {'FP':>6} {'TN':>6} {'Prec':>8} {'Sens':>8} {'Spec':>8} {'#Pos':>8} {'#Neg':>8}")
    print("-" * 82)

    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)

    for i, label in enumerate(class_names):
        tp = np.sum((y_true == i) & (y_pred == i))
        fn = np.sum((y_true == i) & (y_pred != i))
        fp = np.sum((y_true != i) & (y_pred == i))
        tn = np.sum((y_true != i) & (y_pred != i))

        prec = tp / (tp + fp) if (tp + fp) > 0 else 0.0
        sens = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        spec = tn / (tn + fp) if (tn + fp) > 0 else 0.0

        pos = tp + fn
        neg = tn + fp

        print(f"{label:<6} {tp:6d} {fn:6d} {fp:6d} {tn:6d} {prec:8.2f} {sens:8.2f} {spec:8.2f} {pos:8d} {neg:8d}")

    print("-" * 82)


# ────────────────────────────────────────────────
# Cached Feature Extraction (V9 2-class)
# ────────────────────────────────────────────────

def load_or_extract_features(force_extract: bool = False):
    """Extract or load 2-class features.

    Windows are kept only when EVERY sample has label in {1, 2}. The per-window
    label is the max over the window (same rule as V6). Then labels are remapped:
    1 → 0 (clot), 2 → 1 (wall) so they match the 2-output GRU head.
    """
    CACHE_DIR.mkdir(parents=True, exist_ok=True)

    # V9-specific cache filename to avoid clashing with V6's 3-class cache.
    cache_filename = f"features_V10_2class_w{WINDOW_SEC:.1f}s_s{STRIDE_SAMPLES}_seq{SEQ_LEN}_{FEATURE_SET}.npz"
    CACHE_FILE = CACHE_DIR / cache_filename

    if CACHE_FILE.exists() and not force_extract:
        print(f"Loading cached features from: {CACHE_FILE}")
        data = np.load(CACHE_FILE)
        X_seq = data['X_seq']
        y = data['y'].astype(np.int64)
        groups = data['groups']
        print(f"Loaded: {X_seq.shape[0]} sequences | shape={X_seq.shape}")
    else:
        print("Extracting features (V9 2-class: clot/wall only)...")
        data_files = sorted(DATA_DIR.glob("*.parquet"))
        all_data = [pd.read_parquet(f).assign(run_id=f.stem) for f in data_files]
        df_all = pd.concat(all_data, ignore_index=True)

        def _extract_one_run(run_id, group):
            """Extract features and build 2-class sequences for a single run."""
            resistance = group['magRLoadAdjusted'].to_numpy(dtype=np.float32)
            label_array = group['label'].values.astype(np.int64)
            # V9: only accept windows fully covered by clot(1) or wall(2).
            valid_mask = np.isin(label_array, [1, 2])

            if len(resistance) < WINDOW_SAMPLES:
                return [], [], []

            # Precompute full-run EMA arrays (cumulative from run start,
            # including blood samples, to match live streaming EMA state).
            r0 = float(resistance[0])
            ema_f_all, _ = lfilter(_B_FAST, _A_FAST, resistance.astype(np.float64),
                                   zi=[r0 * (1.0 - ALPHA_FAST)])
            ema_s_all, _ = lfilter(_B_SLOW, _A_SLOW, resistance.astype(np.float64),
                                   zi=[r0 * (1.0 - ALPHA_SLOW)])

            extraction_indices = np.arange(WINDOW_SAMPLES - 1, len(resistance), STRIDE_SAMPLES)

            extractor = ClotFeatureExtractor(sample_rate=150, window_sec=WINDOW_SEC,
                                             active_features=active_idx)

            run_features = []
            run_labels = []  # holds remapped 2-class labels: 0=clot, 1=wall

            for idx in extraction_indices:
                win_start = idx - WINDOW_SAMPLES + 1

                # V9: skip windows containing any non-clot/wall samples.
                if not valid_mask[win_start : idx + 1].all():
                    continue

                window_data = resistance[win_start : idx + 1]

                feats = extractor.compute_features_from_array(
                    window_data, float(ema_f_all[idx]), float(ema_s_all[idx]))
                if feats is not None and len(feats) == active_dim:
                    # window_label in {1, 2} (max of window); remap to {0, 1}.
                    window_label_src = int(label_array[win_start : idx + 1].max())
                    if window_label_src == 1:
                        remapped = 0     # clot
                    elif window_label_src == 2:
                        remapped = 1     # wall
                    else:
                        continue         # defensive: valid_mask already filtered

                    run_features.append(feats)
                    run_labels.append(remapped)

            # Build sequences for this run.
            seq_list = []
            labels_list = []
            groups_list = []
            for i in range(SEQ_LEN - 1, len(run_features)):
                seq = np.array(run_features[i - SEQ_LEN + 1 : i + 1])
                seq_list.append(seq)
                labels_list.append(run_labels[i])
                groups_list.append(run_id)

            return seq_list, labels_list, groups_list

        # Parallel extraction across runs
        run_groups = [(run_id, group) for run_id, group in df_all.groupby('run_id')]
        n_jobs = min(os.cpu_count() or 4, len(run_groups))
        print(f"  Parallelizing across {len(run_groups)} runs with {n_jobs} workers...")

        results = Parallel(n_jobs=n_jobs, prefer="processes")(
            delayed(_extract_one_run)(run_id, group) for run_id, group in run_groups
        )

        seq_list = []
        labels_list = []
        groups_list = []
        for seqs, labels, groups_r in results:
            seq_list.extend(seqs)
            labels_list.extend(labels)
            groups_list.extend(groups_r)

        if not seq_list:
            print("ERROR: no V9 sequences extracted. Check that training data contains clot/wall labels.")
            sys.exit(1)

        X_seq = np.array(seq_list, dtype=np.float32)
        y = np.array(labels_list, dtype=np.int64)
        groups = np.array(groups_list)

        print(f"Extracted: {X_seq.shape[0]} sequences | shape={X_seq.shape} from {len(np.unique(groups))} runs")

        # V9 class balance report
        n_clot = int((y == 0).sum())
        n_wall = int((y == 1).sum())
        n_tot = n_clot + n_wall
        print(f"  Class balance: clot={n_clot} ({100*n_clot/max(n_tot,1):.1f}%)  "
              f"wall={n_wall} ({100*n_wall/max(n_tot,1):.1f}%)")

        print("Caching features...")
        np.savez_compressed(CACHE_FILE, X_seq=X_seq, y=y, groups=groups)
        print(f"Saved cache → {CACHE_FILE}")

    # Scaling
    print("Loading V9 scaler and scaling sequences...")
    if not SCALER_PATH.exists():
        print(f"ERROR: V9 scaler not found at {SCALER_PATH}")
        print("Run src/data/fit_scaler_V10.py first.")
        sys.exit(1)
    scaler = joblib.load(SCALER_PATH)
    N, S, F = X_seq.shape
    X_flat = X_seq.reshape(-1, F)
    X_scaled_flat = scaler.transform(X_flat)
    X_scaled = X_scaled_flat.reshape(N, S, F)

    # Check for NaN/Inf from zero-variance features
    nan_count = np.isnan(X_scaled).sum()
    inf_count = np.isinf(X_scaled).sum()
    if nan_count > 0 or inf_count > 0:
        print(f"  WARNING: {nan_count} NaN, {inf_count} Inf values in scaled data — replacing with 0")
        X_scaled = np.nan_to_num(X_scaled, nan=0.0, posinf=0.0, neginf=0.0)

    assert np.abs(X_scaled.mean()) < 0.5, f"Scaling drift — mean={X_scaled.mean():.4f}"
    print(f"Scaling check: mean={X_scaled.mean():.4f}  std={X_scaled.std():.4f}")

    return X_scaled, y, groups, scaler


# ────────────────────────────────────────────────
# Class Weights (V9: 2 classes)
# ────────────────────────────────────────────────

def compute_final_class_weights(y):
    """V9: 2-class weights [clot, wall]. Uses sklearn balanced; overridden by
    CLINICAL_WEIGHTS entries that are > 1.0.
    """
    print("Computing class weights (2-class)...")
    unique = np.unique(y)
    balanced = compute_class_weight('balanced', classes=unique, y=y)
    w_dict = dict(zip(unique, balanced))

    weights = np.ones(NUM_CLASSES)
    for cls, w in w_dict.items():
        weights[int(cls)] = w

    clinical = np.array(CLINICAL_WEIGHTS)
    final_w = np.where(clinical > 1.0, clinical, weights)

    print(f"APPLIED weights: clot={final_w[0]:.2f} wall={final_w[1]:.2f}")
    return torch.tensor(final_w, dtype=torch.float32)


# ────────────────────────────────────────────────
# Focal Loss
# ────────────────────────────────────────────────

class FocalLoss(nn.Module):
    """Focal loss (Lin et al., 2017) for multi-class classification.
    Down-weights easy examples by factor (1-p_t)^gamma.  gamma=0 → CrossEntropyLoss.
    """
    def __init__(self, weight=None, gamma=2.0, reduction='mean'):
        super().__init__()
        self.gamma = gamma
        self.reduction = reduction
        self.ce = nn.CrossEntropyLoss(weight=weight, reduction='none')

    def forward(self, logits, targets):
        ce_loss = self.ce(logits, targets)
        p_t = torch.exp(-ce_loss)
        focal_weight = (1.0 - p_t) ** self.gamma
        loss = focal_weight * ce_loss
        if self.reduction == 'mean':
            return loss.mean()
        elif self.reduction == 'sum':
            return loss.sum()
        return loss


# ────────────────────────────────────────────────
# Training fold
# ────────────────────────────────────────────────

def train_fold(model, train_loader, val_loader, class_weights):
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=5)

    if LOSS_FN == "focal":
        criterion = FocalLoss(weight=class_weights.to(DEVICE), gamma=FOCAL_GAMMA)
        print(f"  Using Focal Loss (gamma={FOCAL_GAMMA})")
    else:
        criterion = nn.CrossEntropyLoss(
            weight=class_weights.to(DEVICE),
            label_smoothing=SMOOTHING_FACTOR)

    best_f1 = 0
    best_state = None
    patience_cnt = 0

    for epoch in range(1, N_EPOCHS + 1):
        model.train()
        for xb, yb in train_loader:
            xb, yb = xb.to(DEVICE), yb.to(DEVICE)
            logits, _ = model(xb, None)
            loss = criterion(logits, yb)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

        model.eval()
        preds, trues = [], []
        with torch.no_grad():
            for xb, yb in val_loader:
                logits, _ = model(xb.to(DEVICE), None)
                preds.extend(logits.argmax(1).cpu().numpy())
                trues.extend(yb.cpu().numpy())

        acc = (np.array(preds) == trues).mean()
        f1_macro = f1_score(trues, preds, average='macro', zero_division=0)
        scheduler.step(f1_macro)

        print(f"Epoch {epoch:3d} | Acc {acc:.4f} | F1 {f1_macro:.4f}", end="")
        if f1_macro > best_f1:
            best_f1 = f1_macro
            best_state = model.state_dict().copy()
            patience_cnt = 0
            print("  ← new best")
        else:
            patience_cnt += 1
            print("")

        if patience_cnt >= PATIENCE:
            print(f"Early stop at epoch {epoch}")
            break

    return best_state, best_f1


# ────────────────────────────────────────────────
# Main — Multiple Seeds Loop
# ────────────────────────────────────────────────

def main():
    set_print_options()

    print("=" * 70)
    print("STARTING V9 TRAINING (2-class: clot vs wall)")
    print("=" * 70)
    print(f"Device               : {DEVICE}")
    print(f"FEATURE_SET          : {FEATURE_SET}")
    print(f"Active features      : {active_dim}  ({FEATURE_SET})")
    print(f"SEQ_LEN              : {SEQ_LEN}")
    print(f"NUM_CLASSES          : {NUM_CLASSES}  ({CLASS_NAMES})")
    print(f"SEEDS_TO_TRY         : {SEEDS_TO_TRY}")
    print(f"Batch size           : {BATCH_SIZE}")
    print(f"Num workers          : {NUM_WORKERS}")
    print(f"Patience             : {PATIENCE}")
    print("-" * 70)

    force_extract = "--force-extract" in sys.argv or "-f" in sys.argv
    if force_extract:
        print("Force re-extraction requested.")

    X_scaled, y, groups, scaler = load_or_extract_features(force_extract=force_extract)

    # Drop unlabeled sequences (label == -1) from training. Should be zero after
    # V9 filtering, but keep for safety.
    valid = (y != -1)
    n_dropped = (~valid).sum()
    if n_dropped > 0:
        print(f"Dropping {n_dropped} unlabeled sequences (label == -1) "
              f"out of {len(y)} total ({100*n_dropped/len(y):.1f}%)")
        X_scaled = X_scaled[valid]
        y = y[valid]
        groups = groups[valid]

    # Sanity: labels must all be in {0, 1} for the 2-class head.
    unique_labels = np.unique(y)
    if not set(unique_labels.tolist()).issubset({0, 1}):
        print(f"ERROR: V9 training expects labels in {{0, 1}} but got {unique_labels}")
        sys.exit(1)

    class_weights = compute_final_class_weights(y)

    n_groups = len(np.unique(groups))
    n_splits = min(5, n_groups)
    print(f"\nGroupKFold: {n_splits} splits on {n_groups} groups")

    gkf = GroupKFold(n_splits=n_splits)

    best_global_f1 = 0.0
    best_state_global = None
    best_seed = None

    for seed in SEEDS_TO_TRY:
        print(f"\n{'='*60}")
        print(f"Training with SEED = {seed}")
        print(f"{'='*60}")

        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False
        torch.use_deterministic_algorithms(True)

        best_f1_this_seed = 0.0
        best_state_this_seed = None

        for fold, (train_idx, val_idx) in enumerate(gkf.split(X_scaled, y, groups), 1):
            print(f"\nFold {fold}/{n_splits}")

            tr_X, va_X = X_scaled[train_idx], X_scaled[val_idx]
            tr_y, va_y = y[train_idx], y[val_idx]

            # Mild oversampling of the minority class (wall). Clot is the
            # majority class in the 2-class setup, so no clot oversampling.
            wall_idx = np.where(tr_y == 1)[0]
            repeat_wall = 1
            if len(wall_idx) > 0 and repeat_wall > 0:
                tr_X = np.concatenate([tr_X] + [tr_X[wall_idx]] * repeat_wall)
                tr_y = np.concatenate([tr_y] + [tr_y[wall_idx]] * repeat_wall)

            train_ds = TensorDataset(torch.from_numpy(tr_X).float(), torch.from_numpy(tr_y).long())
            val_ds   = TensorDataset(torch.from_numpy(va_X).float(), torch.from_numpy(va_y).long())

            g = torch.Generator().manual_seed(seed + fold)

            train_dl = DataLoader(
                train_ds,
                batch_size=BATCH_SIZE,
                shuffle=True,
                num_workers=NUM_WORKERS,
                pin_memory=PIN_MEMORY,
                persistent_workers=True if NUM_WORKERS > 0 else False,
                generator=g,
            )

            val_dl = DataLoader(
                val_ds,
                batch_size=BATCH_SIZE,
                shuffle=False,
                num_workers=NUM_WORKERS,
                pin_memory=PIN_MEMORY,
                persistent_workers=True if NUM_WORKERS > 0 else False
            )

            # ClotGRU from gru_torch_V9 has output_size=2 by default.
            model = ClotGRU().to(DEVICE)
            state, fold_f1 = train_fold(model, train_dl, val_dl, class_weights)

            if fold_f1 > best_f1_this_seed:
                best_f1_this_seed = fold_f1
                best_state_this_seed = state

        print(f"\nSeed {seed} best F1-macro: {best_f1_this_seed:.4f}")

        # Save every seed (V9 naming)
        if best_state_this_seed is not None:
            model_filename = f"clot_gru_trained_V10_2class_seq{SEQ_LEN}_{FEATURE_SET}_seed{seed}_f1{best_f1_this_seed:.4f}.pt"
            save_path = PROJECT_ROOT / "src" / "training" / model_filename

            torch.save(best_state_this_seed, save_path)

            print(f"   ✅ Saved V10 model for seed {seed}")
            print(f"      Filename: {model_filename}")
            print(f"      F1-macro: {best_f1_this_seed:.4f}")
            print(f"      Path: {save_path}")
        else:
            print(f"   ⚠️  No model saved for seed {seed} (best_state was None)")

        if best_f1_this_seed > best_global_f1:
            best_global_f1 = best_f1_this_seed
            best_state_global = best_state_this_seed
            best_seed = seed
            print(f"   → New global best! (Seed {seed})")

    # Save overall best (V9 naming)
    if best_state_global is not None:
        latest_path = PROJECT_ROOT / "src" / "training" / f"clot_gru_trained_V10_2class_seq{SEQ_LEN}_{FEATURE_SET}.pt"
        torch.save(best_state_global, latest_path)
        print(f"\n✅ Also saved overall best as: clot_gru_trained_V10_2class_seq{SEQ_LEN}_{FEATURE_SET}.pt")

        generic_path = PROJECT_ROOT / "src" / "training" / "clot_gru_trained_V10_2class.pt"
        torch.save(best_state_global, generic_path)
        print(f"✅ Also saved as: clot_gru_trained_V10_2class.pt (default inference path)")

    print("\n" + "="*70)
    print("V10 TRAINING FINISHED")
    print("="*70)
    print(f"Global best F1-macro: {best_global_f1:.4f} (Seed {best_seed})")

    # ── Auto-save versioned snapshot (V9-specific saver) ──
    try:
        from src.data.save_version_V10 import save_version_v10
        save_version_v10(f1=best_global_f1, note=f"V10 2-class best seed {best_seed}")
    except Exception as e:
        print(f"(save_version_v10 skipped: {e})")


if __name__ == "__main__":
    main()

# %%
