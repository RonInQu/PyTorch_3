# fit_scaler_V10.py
"""
Fits the StandardScaler for clot detection features — V10 (2-class).
...
Output filename is distinct from V6 so both scalers can coexist:
    clot_feature_scaler_V10_2class_seq{SEQ_LEN}_{dim_str}.pkl
"""
...
# ================= IMPORT FROM gru_torch_V10 =================
# V9 uses the same feature set / extractor as V6/V9; only the classification head changes.
from src.models.gru_torch_V10 import FEATURE_SET, TOTAL_FEATURES, \
...
from src.training.train_gru_V10 import STRIDE_SAMPLES

# ================= Derived CONFIG =================
OUTPUT_SCALER_PATH = PROJECT_ROOT / "src" / "data" / f"clot_feature_scaler_V10_2class_seq{SEQ_LEN}_{dim_str}.pkl"

print("Scaler config (V10 / 2-class):")
...
print(f"\n✅ V10 (2-class) scaler fitted and saved → {OUTPUT_SCALER_PATH}")
...
plt.title(f"V10 (2-class) Feature Correlation Heatmap ({X_scaled.shape[1]} features)")
plt.tight_layout()
plt.savefig("feature_correlation_heatmap_V10_2class.png", dpi=300, bbox_inches='tight')
plt.close()
print("   Saved → feature_correlation_heatmap_V10_2class.png")

print("\nV10 scaler fitting complete.")
