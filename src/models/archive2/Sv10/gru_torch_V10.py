# gru_torch_V10.py
"""
Real-time clot detection — V10 (2-class: clot vs wall)

Architecture:
    - GRU model outputs 2 logits -> softmax -> [P(clot), P(wall)].
    - Blood is never predicted by the model. It is emitted only when the
        detector-assisted (DA) logic tree says blood.
    - Downstream code still receives a 3-vec [P(blood), P(clot), P(wall)] for
        backward compatibility:
            * DA=blood      -> emit [1, 0, 0] (hard reset)
            * DA=clot/wall  -> DA wins by default; ML overrides only when the
...
Stability gate (ML override rights):
    ...
    can be required to confirm the same non-DA class at high confidence before
...
Feature set: the same 21-feature clot_wall_focused subset used by the V9
2-class clot-vs-wall path. Scaler, cache, and model artifacts are V10-tagged.
"""
...
# V10 keeps DA only as a blood signal and for reporting/comparison.
...
MODEL_PATH = PROJECT_ROOT / "src" / "training" / "clot_gru_trained_V10_2class.pt"
    ...
    def __init__(self, model_path=MODEL_PATH, scaler_path=SCALER_PATH,
        ...
        if ensemble_seeds:
            ...
            for seed in ensemble_seeds:
                # V10 (2-class) model naming
                pattern = f"clot_gru_trained_V10_2class_seq{SEQ_LEN}_{FEATURE_SET}_seed{seed}_f1*.pt"
...
def main():
    parser = argparse.ArgumentParser(description="Run V10 GRU inference on test parquet files.")
    parser.add_argument("--file", type=str, default=None,
                        help="Optional single parquet file to process (absolute path or relative to TEST_DATA_DIR).")
    parser.add_argument("--glob", type=str, default=None,
                        help="Optional glob pattern inside TEST_DATA_DIR, for example '*CENT0231*'.")
    args = parser.parse_args()

    default_glob = "*_labeled_segment_denoised.parquet" if USE_DENOISED else "*_labeled_segment.parquet"

    if args.file:
        file_path = Path(args.file)
        if not file_path.is_absolute():
            file_path = TEST_DATA_DIR / file_path
        files = [file_path]
    else:
        glob_pattern = args.glob or default_glob
