"""
save_version_V10.py — Snapshot a trained V10 (2-class) pipeline version.
...
def save_version_v10(
    ...
    from src.models.gru_torch_V10 import (
    ...
    from src.training.train_gru_V10 import (
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"V10 model not found: {MODEL_PATH}")
    if not SCALER_PATH.exists():
        raise FileNotFoundError(f"V10 scaler not found: {SCALER_PATH}")

    # ── build version folder name: <timestamp>_v9[_tag] ──
    timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    folder_name = f"{timestamp}_v10"
    if tag:
    ...
    script_copies = [
        PROJECT_ROOT / "src" / "data" / "fit_scaler_V10.py",
        PROJECT_ROOT / "src" / "training" / "train_gru_V10.py",
        PROJECT_ROOT / "src" / "models" / "gru_torch_V10.py",
        PROJECT_ROOT / "src" / "data" / "save_version_V10.py",
    ...
    seed_pattern = f"clot_gru_trained_V10_2class_seq{SEQ_LEN}_{FEATURE_SET}_seed*.pt"
    seed_files = sorted(
    ...
    # ── write manifest ──
    lines: list[str] = []
    lines.append(f"# V10 Pipeline version — {timestamp}")
    lines.append(f"# Architecture: 2-class (clot vs wall) with DA guardrail and no ML override")
    ...
    lines.append("V10 note: model trains on {clot=1, wall=2} only; blood windows skipped.")
    lines.append("")

    lines.append("=== TRAINING CONFIG (V10) ===")
    ...
    lines.append("=== INFERENCE CONFIG (V10) ===")
    ...
    lines.append("")
    lines.append("Override path:        disabled")
    lines.append("")
    ...
    print(f"\n{'=' * 60}")
    print(f"  V10 VERSION SAVED  →  {version_dir.relative_to(PROJECT_ROOT)}")
    print(f"{'=' * 60}")
