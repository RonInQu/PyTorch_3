# #%%
# train_gru_V6_Normalized.py
"""
Training script for clot detection — V6 Normalized (GRU on features).
Uses division-normalized data: R_norm = (R - baseline) / baseline.
Input column: 'R_normalized' (dimensionless).
"""
...
# Import from gru_torch_V6_Normalized (single source of truth)
from src.models.gru_torch_V6_Normalized import ClotFeatureExtractor, ClotGRU, \
...
def load_or_extract_features(force_extract: bool = False):
    ...
    cache_filename = f"features_normalized_w{WINDOW_SEC:.1f}s_s{STRIDE_SAMPLES}_seq{SEQ_LEN}_{FEATURE_SET}.npz"
    ...
    if CACHE_FILE.exists() and not force_extract:
        ...
        print("Extracting features (normalized data)...")
        ...
        def _extract_one_run(run_id, group):
            """Extract features and build sequences for a single run."""
            resistance = group['R_normalized'].to_numpy(dtype=np.float32)
...
def main():
    ...
    print("=" * 70)
    print("STARTING TRAINING (NORMALIZED)")
    print("=" * 70)
    ...
    print(f"Input column         : R_normalized")
    print("-" * 70)
    ...
    for seed in SEEDS_TO_TRY:
        ...
        if best_state_this_seed is not None:
            model_filename = f"clot_gru_trained_normalized_seq{SEQ_LEN}_{FEATURE_SET}_seed{seed}_f1{best_f1_this_seed:.4f}.pt"
    ...
    if best_state_global is not None:
        latest_path = PROJECT_ROOT / "src" / "training" / f"clot_gru_trained_normalized_seq{SEQ_LEN}_{FEATURE_SET}.pt"
        torch.save(best_state_global, latest_path)
        print(f"\n✅ Also saved overall best as: clot_gru_trained_normalized_seq{SEQ_LEN}_{FEATURE_SET}.pt")

        generic_path = PROJECT_ROOT / "src" / "training" / "clot_gru_trained_normalized.pt"
        torch.save(best_state_global, generic_path)
        print(f"✅ Also saved as: clot_gru_trained_normalized.pt")
